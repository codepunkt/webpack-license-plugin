...
class WebpackLicensePlugin {
  ...
  handleEmit(compilation, callback) {
    ...
    const packageVersions = map(packageMeta, ({ name, path }) => {
      const packageJson = JSON.parse(fs.readFileSync(join(path, 'package.json'), 'UTF-8'))
    ...
    exec(
      ...
      (err, stdout, stderr) => {
        if (err) {
          const match = stderr.match(/Found license defined by the --failOn flag: "(.*)"/i)
          if (match && match[1]) {
            compilation.errors.push(
              new Error(`WebpackLicensePlugin: found blacklisted license "${match[1]}"`),
            )
          }
          return callback()
        }
